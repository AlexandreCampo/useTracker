...
// 	    // stay within bounds of video frame
// 	    if (fy >=0 && fy < videoHeight && fx >= 0 && fx < videoWidth)
// 	    {
// 		unsigned char* frameRow = (unsigned char*) frame->data[0] + fy * frame->linesize[0];
// 		int v = frameRow[fx*3];
// 		v+= frameRow[fx*3+1];
// 		v+= frameRow[fx*3+2];
// 		pattern[x+y*pwidth] = (unsigned char)(v/3);
...
//     // save for debug
//     for (int x = 0; x < pwidth; x++) pattern [x] = p[x];

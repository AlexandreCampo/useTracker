...
//(*InternalHeaders(DialogWatershed)
#include <wx/string.h>
#include <wx/intl.h>
//*)


//(*IdInit(DialogWatershed)
const long DialogWatershed::ID_STATICTEXT1 = wxNewId();
const long DialogWatershed::ID_SPINCTRL_THRESHOLD = wxNewId();
const long DialogWatershed::ID_RADIOBOX1 = wxNewId();
const long DialogWatershed::ID_CHECKBOX1 = wxNewId();
const long DialogWatershed::ID_STATICTEXT2 = wxNewId();
const long DialogWatershed::ID_SPINCTRL_ZONE = wxNewId();
const long DialogWatershed::ID_BUTTON1 = wxNewId();
//*)

BEGIN_EVENT_TABLE(DialogWatershed,wxDialog)
	//(*EventTable(DialogWatershed)
	//*)
END_EVENT_TABLE()

DialogWatershed::DialogWatershed(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
	//(*Initialize(DialogWatershed)
	...
	Connect(ID_SPINCTRL_THRESHOLD,wxEVT_COMMAND_SPINCTRL_UPDATED,(wxObjectEventFunction)&DialogWatershed::OnSpinCtrlThresholdChange);
	Connect(ID_RADIOBOX1,wxEVT_COMMAND_RADIOBOX_SELECTED,(wxObjectEventFunction)&DialogWatershed::OnRadioBox1Select);
	Connect(ID_CHECKBOX1,wxEVT_COMMAND_CHECKBOX_CLICKED,(wxObjectEventFunction)&DialogWatershed::OnCheckBoxRestrictToZoneClick);
	Connect(ID_SPINCTRL_ZONE,wxEVT_COMMAND_SPINCTRL_UPDATED,(wxObjectEventFunction)&DialogWatershed::OnSpinCtrlZoneChange);
	Connect(ID_BUTTON1,wxEVT_COMMAND_BUTTON_CLICKED,(wxObjectEventFunction)&DialogWatershed::OnButtonOkClick);
	//*)
...
DialogWatershed::~DialogWatershed()
{
	//(*Destroy(DialogWatershed)
	//*)
}

void DialogWatershed::SetPlugin (std::vector<PipelinePlugin*> pfv)
{
    for (auto func : pfv)
	plugin.push_back(dynamic_cast<Watershed*> (func));
...
void DialogWatershed::OnButtonCancelClick(wxCommandEvent& event)
{
    this->Hide();
}

void DialogWatershed::OnButtonOkClick(wxCommandEvent& event)
{
//    OnButtonApplyClick(event);
    this->Hide();
}

void DialogWatershed::OnButtonApplyClick(wxCommandEvent& event)
{
}


void DialogWatershed::OnCheckBoxRestrictToZoneClick(wxCommandEvent& event)
{
    for (auto f : plugin)
    	f->restrictToZone = event.IsChecked();
}

void DialogWatershed::OnRadioBox1Select(wxCommandEvent& event)
{
    for (auto f : plugin)
    	f->additive = RadioBox1->GetSelection();
}

void DialogWatershed::OnSpinCtrlZoneChange(wxSpinEvent& event)
{
    for (auto f : plugin)
    	f->zone = SpinCtrlZone->GetValue();
}

void DialogWatershed::OnSpinCtrlThresholdChange(wxSpinEvent& event)
{
    for (auto f : plugin)

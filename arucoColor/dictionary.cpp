...
namespace ac
{
    ...
    void Dictionary::refreshHues()
    {	
	// build a lookup table to quickly convert hue to ref color
	huesToColor.clear();
	for (int i = 0; i <= 180*3; i++)
	    huesToColor.push_back(0);

	if (Marker::referenceHues.size() == 0)
	    return;
	
	vector<int> referenceHues = Marker::referenceHues;
    ...
    {
	// start from original codes
	vector<pair<int,int>> oc = originalCodes;
    ...
    {
	// make sure markers are sorted and reference hues refreshed
	if (!updated)
	{
	    update();
	}
	...
	for (auto c : candidates)
	{
	    ...
	    for (int i = Marker::numPixels - 1; i >= 0; i--)
	    {
		code += c[i] * exponent;
...
//    cout << "Best = " << best << " code " << bestCode << " | "; // endl;
    ...
    std::vector<std::pair<int,int>> Dictionary::getCodes()
    {
